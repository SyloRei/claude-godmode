...
Scan for "Auto Mode Active" (case-insensitive). When detected: pick the
safest first step (smallest atomic refactor); never batch multiple steps;
surface assumptions inline. Quality gates between every step are
non-negotiable even under Auto Mode. See `rules/godmode-skills.md` § Auto
Mode Detection.
...
- **@architect** — for design review on large-scope refactorings
